...
public class GourmetCoffee  {
	...
	public void displayCatalog() {
		...
		if (size == 0) {
            ...
            Iterator<Product> ite = this.catalog.iterator();
			while(ite.hasNext()) {
	...
	public void displayOrder() {
		...
		if (size == 0) {
            ...
            Iterator<OrderItem> ite = currentOrder.iterator();
			while (ite.hasNext()) {
                OrderItem orderItem = ite.next();
	...
	public void displayOrdersSold() {
		...
		if (numOrders != 0) {
			int orderNumber = 1;
            Iterator<Order> ite = sales.iterator();
			while (ite.hasNext()) {
                Order order = ite.next();
				stdOut.println("Order " + orderNumber++);
                Iterator<OrderItem> inFunc_ite = order.iterator();
				while (inFunc_ite.hasNext()) {
                    OrderItem orderItem = inFunc_ite.next();
					stdOut.println("   " + orderItem.toString());
				}
				stdOut.println("   Total: " +
	...
	public void displayNumberOfOrders(Product product) {
        ...
        Iterator<Order> ite = sales.iterator();
		while (ite.hasNext()) {
			Order orders = ite.next();
            Iterator<OrderItem> inFunc_ite = orders.iterator();
			while (inFunc_ite.hasNext()) {
				OrderItem orderItem = inFunc_ite.next();
	...
	public void displayTotalQuantityOfProducts() {
		...
		/* PLACE YOUR CODE HERE */
		int sum;
        Iterator<Product> ite = catalog.iterator();
		while (ite.hasNext()) {
			sum = 0;
			Product product = ite.next();
            Iterator<Order> sales_ite = sales.iterator();
			while (sales_ite.hasNext()) {
				Order orders = sales_ite.next();
                Iterator<OrderItem> item_ite = orders.iterator();
